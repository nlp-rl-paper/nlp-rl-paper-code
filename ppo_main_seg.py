...
from ActorCriticModelVizCNN import ActorCriticModel as ActorCriticModel
...
def get_free_gpu():
    import time
    import random
    time.sleep(5 + 2*random.randint(5,30))
...
def main():
    ...
    pg = PlotGenerator("ppo",args.scenario, args.arch, False, 1, args.seed)
    ...
    envs = make_vec_envs(args.env_name, args.rep_type ,resolution ,args.seed, args.scenario ,args.num_processes,
...
#     actor_critic.to(device)
    ...
    for j in range(num_updates):
        ...
        for step in range(args.num_steps):
            # Sample actions
            with torch.no_grad():
                inputs = rollouts.obs[step].reshape([-1,args.n_channels,resolution[0],resolution[1]])
        ...
        with torch.no_grad():
            next_value = actor_critic.get_value(
                rollouts.obs[-1].reshape([-1,args.n_channels,resolution[0],resolution[1]]),
        ...
        if args.gail:
            ...
            for step in range(args.num_steps):
                rollouts.rewards[step] = discr.predict_reward(
                    rollouts.obs[step].reshape([-1,args.n_channels,resolution[0],resolution[1]]), rollouts.actions[step], args.gamma,
                    rollouts.masks[step])
