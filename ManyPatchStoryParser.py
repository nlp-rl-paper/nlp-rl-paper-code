...
#generates an empty dictionary in the following format {-3: 0, -2: 0, -1: 0, 0: 0, 1: 0, 2: 0, 3: 0}
def _build_dict(patch_count):
    keys = list(range(-1*patch_count//2 + 1,patch_count//2 + 1))
    values = [0 for _ in range(-1 * patch_count // 2 + 1, patch_count // 2 + 1)]
    return dict(zip(keys,values))


#geneates an empty patch list dict
def _build_patch_dict(patch_count,dist_level_count):
    return [_build_dict(patch_count) for _ in range(0,dist_level_count)]


def _build_patch_str_dict(patch_count):
    keys = list(range(-1 * patch_count // 2 + 1, patch_count // 2 + 1))
    values = []
    for i in keys:
        if i < 0:
            values.append(" left " + str(-1*i)+ " patch ")
        elif i == 0:
            values.append("front patch")
        else:
            values.append(" right " + str(i) + " patch ")
    return dict(zip(keys,values))

def _build_dist_str_dict(dist_level_count):
    keys = list(range(0,dist_level_count))
    values = [str(i) + " range zone " for i in keys]
    return dict(zip(keys,values))
...
class StoryParser(Parser):
    def __init__(self,hist_size,patches_count,dist_threshold_count,green_monster=False):
        ...
        self.DISTANCE_THRESHOLDS_COUNT = dist_threshold_count
        self.green_monster = green_monster
    ...
    def calc_state_string(self,cur_game_state : GameState ,game_variable : GameVariable):
        ...
        DISTANCE_THRESHOLDS_COUNT = self.DISTANCE_THRESHOLDS_COUNT

        #enemy counter dictionaries: 0 - close, 1 - mid , 2 - far
        fireball_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        ranged_enemy_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        melee_enemy_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        medkit_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        armor_red_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        armor_blue_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        armor_green_patch_counter =_build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        weapons_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        ammo_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        medkit_medkit_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        medkit_stimpack_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        obstacles_monsters_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        obstacles_barrels_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        obstacles_trees_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        obstacles_green_pillars_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        obstacles_red_pillars_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        invul_patch_counter = _build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)
        stealth_patch_counter =_build_patch_dict(PATCHES_COUNT,DISTANCE_THRESHOLDS_COUNT)

        patch_str_dict = _build_patch_str_dict(PATCHES_COUNT)
        patch_dist_str_dict = _build_dist_str_dict(DISTANCE_THRESHOLDS_COUNT)
        ...
        for dist in range(0,self.DISTANCE_THRESHOLDS_COUNT):
            for i in range(-1*self.PATCHES_COUNT // 2 + 1, self.PATCHES_COUNT // 2 + 1):
                patch_dist_counter = 0
                if melee_enemy_patch_counter[dist][i] > 0:
                    patch_dist_counter += 1
                    if self.green_monster == True:
                        color = "green"
                    else:
                        color = "red"
                    if melee_enemy_patch_counter[dist][i] == 1:
                            state_str += " there is a " + color + " melee enemy,"
                    else:
                        state_str += " there are " + str(melee_enemy_patch_counter[dist][i]) + " red melee enemies,"
    ...
    def calc_patch(self, x):
        W = self.PATCHES_COUNT // 2
        for i in range(-1 * W + 1, W + 1):
            if x < (i+W+1)*SCREEN_WIDTH // (2*W + 1):
                return i,""
        return -1,""
